...
class ResponsesAgentThreadActions:
    ...
    async def invoke(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            verbosity: The verbosity level (only supported by GPT-5 family models) controlling how much internal
                model reasoning is surfaced in the natural language response ("low", "medium", or "high").
    ...
    def _generate_options(cls: type[_T], **kwargs: Any) -> dict[str, Any]:
        ...
        verbosity = merged.get("verbosity", None)
        ...
        # Build text configuration with verbosity if provided
        text_config = merged.get("text")
        if verbosity is not None:
            if text_config is None:
                text_config = {"verbosity": verbosity}
            elif isinstance(text_config, dict):
                text_config = {**text_config, "verbosity": verbosity}
            else:
                # If text_config is not a dict, we need to convert it appropriately
                text_config = {"verbosity": verbosity}
