...
These tests align with C# Semantic Kernel patterns and verify the reasoning functionality
for OpenAI ResponsesAgent, following the same test patterns as the C# implementation.
"""
...
class TestOpenAIResponsesAgentReasoning:
    """Tests for OpenAI ResponsesAgent reasoning effort configuration, aligned with C# patterns."""
    ...
    def test_constructor_reasoning_effort_defaults_to_none(self):
        ...
        assert agent._default_reasoning_effort is None
    ...
    def test_constructor_reasoning_effort_validation(self):
        ...
        for valid_effort in ["low", "medium", "high"]:
    ...
    def test_reasoning_priority_order_constructor_used_as_default(self):
        """Test constructor reasoning effort is used as default (C# priority pattern)."""
        # Arrange: Mock agent with constructor reasoning
        agent = AsyncMock()
        agent.ai_model_id = "o1"
        agent._default_reasoning_effort = "low"  # Constructor-level setting
    ...
    def test_reasoning_priority_order_per_invocation_overrides_constructor(self):
        """Test per-invocation reasoning overrides constructor default (C# priority pattern)."""
        # Arrange: Mock agent with constructor default
        agent = AsyncMock()
        agent.ai_model_id = "o1"
        agent._default_reasoning_effort = "low"  # Constructor setting
    ...
    def test_reasoning_priority_order_complete_hierarchy(self):
        ...
        agent = AsyncMock()
        agent.ai_model_id = "o1"
        agent._default_reasoning_effort = "low"
        options = ResponsesAgentThreadActions._generate_options(
            agent=agent,
            model="o1",
        ...
        agent = AsyncMock()
        agent.ai_model_id = "o1"
        agent._default_reasoning_effort = None  # No constructor reasoning
        options = ResponsesAgentThreadActions._generate_options(agent=agent, model="o1")
        assert options["reasoning"]["effort"] == "high"  # Model default for O1

    def test_multi_agent_reasoning_isolation(self):
        """Test multiple agents maintain separate reasoning configurations (C# isolation pattern)."""
        client = AsyncMock(spec=AsyncOpenAI)
        ...
        assert low_agent._default_reasoning_effort == "low"
        assert high_agent._default_reasoning_effort == "high"
    ...
    def test_o_series_model_gets_automatic_reasoning(self):
        """Test O-series models get automatic reasoning effort (aligns with C# behavior)."""
        agent = AsyncMock()
        agent._default_reasoning_effort = None  # No constructor reasoning
    ...
    def test_explicit_none_reasoning_disables_automatic_reasoning(self):
        ...
        agent = AsyncMock()
        agent.ai_model_id = "o1"
        agent._default_reasoning_effort = None
    ...
    def test_reasoning_object_structure_follows_openai_api(self):
        ...
        agent = AsyncMock()
        agent.ai_model_id = "o1"
        agent._default_reasoning_effort = "high"
