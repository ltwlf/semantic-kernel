...
class TestOpenAIResponsesAgentReasoning:
    ...
    # Verbosity (GPT-5 only): ensure value merges into text config and preserves existing keys.
    def test_verbosity_parameter_included_in_text_config(self):
        """Verbosity merges into fresh and existing text configs (GPT-5)."""
        agent = AsyncMock()
        agent.ai_model_id = "gpt-5"
        agent.reasoning_effort = None

        # Fresh config (no existing text)
        options = ResponsesAgentThreadActions._generate_options(agent=agent, model="gpt-5", verbosity="high")
        assert options["text"]["verbosity"] == "high"

        # Merge into existing (non-destructive)
        existing_text = {"format": {"type": "text"}}
        snapshot = dict(existing_text)
        options2 = ResponsesAgentThreadActions._generate_options(
            agent=agent, model="gpt-5", text=existing_text, verbosity="low"
        )
        assert options2["text"]["verbosity"] == "low"
        assert "format" in options2["text"]
        assert existing_text == snapshot

    def test_verbosity_ignored_for_non_supported_models(self):
        """Non-GPT-5 models: verbosity passes through (SDK doesn't block)."""
        agent = AsyncMock()
        agent.ai_model_id = "gpt-4o"  # Does not support verbosity but we allow pass-through (no validation here)
        agent.reasoning_effort = None

        options = ResponsesAgentThreadActions._generate_options(agent=agent, model="gpt-4o", verbosity="medium")
        # Still included; upstream service may ignore.
        assert options["text"].get("verbosity") == "medium"
