...
"""ResponsesAgent reasoning & (GPT-5) verbosity demo.

Shows (OpenAI first, falls back to Azure if not configured):
- Constructor vs per-invocation reasoning_effort (override precedence)
- Reasoning isolation across multiple agents
- Token / summary metadata extraction
- Function calling + reasoning (calculator plugin)
- Complex reasoning + side‑by‑side effort comparison
- GPT‑5‑only verbosity (low|medium|high) while keeping reasoning effort constant (minimal)

Prerequisites:
- OpenAI or Azure OpenAI API access
- Reasoning‑capable model (e.g., gpt-5, o4-mini, o3-mini)
- Env vars (see semantic_kernel/.env.example)

Notes:
- Defaults to GPT‑5 if no chat model id supplied.
- Verbosity is sent ONLY for GPT‑5 series models; other models ignore it.
"""
...
class ReasoningAgentDemo:
    ...
    async def demo_per_invocation_override(self):
        """Per-invocation reasoning override precedence."""
        print("\n" + "=" * 70)
    ...
    async def demo_reasoning_comparison(self):
        """Side-by-side reasoning effort comparison."""
        print("\n" + "=" * 70)
    ...
    async def demo_model_verbosity(self):
        """GPT-5 verbosity parameter (skipped for non GPT-5 models)."""
        # Resolve an agent to inspect model id
        active_agent = self.agent_low or self.agent_high
        if active_agent is None:
            print("Verbosity demo skipped: no agent available.")
            return

        model_id = getattr(active_agent, "ai_model_id", "") or ""
        if not model_id.startswith("gpt-5"):
            print("Verbosity demo skipped: model is not GPT-5 (verbosity unsupported).")
            return

        print("\n" + "=" * 70)
        print("DEMO 5: GPT-5 Verbosity Levels (low vs medium vs high)")
        print("=" * 70)
        prompt = "Explain what a hash table is in one paragraph."
        print(f"Prompt: {prompt}\n")

        for level in ["low", "medium", "high"]:  # Compare breadth of output only
            print(f"Verbosity = {level}")
            print("-" * 30)
            # Keep reasoning effort constant for a clean verbosity contrast
            async for content in active_agent.invoke(
                [ChatMessageContent(role="user", content=prompt)],
                reasoning="minimal",
                verbosity=level,
            ):
                await self._display_response(content, f"verbosity={level}", truncate=400)
                break
            print()

    async def _display_response(self, content, reasoning_type: str, truncate: int | None = None):
        """Print response plus (if present) reasoning metadata."""
        # Reasoning metadata (tokens, summary) if provided
        reasoning_info = {}
    ...
    async def run_demo(self):
        ...
        try:
            ...
            await self.demo_model_verbosity()
            ...
            print("✓ GPT-5 verbosity (if applicable)")
            ...
            print("2. Ensure you're using gpt-5 or o-series models (o1, o3-mini, o4-mini)")
